...
# The method should return a boolean indicating whether or not at least one of the 
# elements of the array returns true when given to the block. 
...
# The method should return a boolean indicating whether or not there are exactly n elements that return true 
...
def filter_out!(arr, &prc)
    ...
    #select - filters but doens't mutate the input array
    #
end
# Examples
...
#where the elements that return true when given to the block come before all of the elements that return false when given to the block.
...
def proctition(arr, &prc)
    new_arr1 = arr.select {|ele|  prc.call(ele)}
    new_arr2 = arr.select {|ele| !prc.call(ele)}
    new_arr1 + new_arr2
end
...
#w ith their return values when they are passed into the second proc. 
# #       ...
...
def selected_map!(arr, proc1, proc2)
    arr.map! { |ele| proc2.call(ele) if proc1.call(ele) }
    nil
end
...
#If the original word returns true when given to the corresponding proc key, append the suffix. 
#However, if an original word returns true for multiple procs, then the suffixes should be appended in the order that they appear in the input hash.

#Examples

def proc_suffix(sent, hash)
    #procs = keys to hash
    #hash values = suffix strings 
    new_sent = []
    arr = sent.split(" ")
    arr.each do |word|

    procs = numProcs(word, hash) 
...
# then it should only be placed into the array that corresponds to the proc that appears first in the arguments.
...
def proctition_platinum(arr, *procs)
    ...
    hash = {}
    #Procs = [1, 2]
    procs.each_with_index do |proc, i| #proc = 1, i = 0
        hash[(i + 1)] = value(arr, proc)
    end
    count = 0
    arr.each do |ele|
        procs.each do |proc|
...
def value(arr, procs) #pass an array and 1 proc from the above procs array
    values = []

    arr.each do |ele|
        values << ele if procs.call(ele)
    end
    values
end
